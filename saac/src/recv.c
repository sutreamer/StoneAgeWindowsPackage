...
#endif
/*
   锟截★拷丞锟接★拷锟斤拷锟斤拷锟绞э拷锟斤拷锟斤拷锟斤拷锟斤拷印锟斤拷锟斤拷锟斤拷锟秸凤拷锟绞э拷锟斤拷锟斤拷锟斤拷拢锟�
   int fd : 锟斤拷锟斤拷socket
                                ...
                                int lock,char* opt , int mesgid)
{
    ...
    // Spock 2000/10/31
    //log( "锟斤拷取锟斤拷锟斤拷: 锟斤拷锟斤拷锟斤拷业 %s %s, 锟斤拷锟斤拷=%d\n", id,pas,process );
    // Spock end
}
...
/*
   ...
   char *id : ID
   char *pas : 锟斤拷锟绞达拷锟斤拷锟斤拷
   char *charname : 平锟揭凤拷  
   char *opt : 锟斤拷锟斤拷锟斤拷锟斤拷锟�  侬  锟斤拷 CharList锟斤拷锟斤拷锟斤拷宥�乒锟斤拷锟狡伙拷忒锟斤拷锟斤拷  
   char *charinfo : 锟斤拷锟斤拷锟教伙拷锟斤拷锟斤拷锟斤拷锟斤拷锟狡斤拷曳锟斤拷锟斤拷锟斤拷锟斤拷锟�  锟斤拷锟斤拷锟斤拷锟斤拷  锟斤拷
   int id : 锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷id
*/
#ifdef _NewSave
...
void saacproto_ACLock_recv( int ti ,char* id,int lock,int mesgid )
{
  ...
  if (lock==2) {
		...
		if ( lockUser( getGSName( ti ) , id , "0" , lock , result,sizeof(result),
										retdata ,sizeof( retdata ) , "0" , "0") < 0 ) {
#endif
			log( "锟斤拷锟斤拷锟矫伙拷: %s 失锟斤拷\n" , id );
		} else {
			log( "锟斤拷锟斤拷锟矫伙拷: %s 锟缴癸拷\n" , id );
		}
  }
    // Spock end
...
{
    if( !is_game_server_login(ti) ){
        log( "锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟剿猴拷:%s status:%d\n",
             id, status );
        return;
    }

	if( status == 0 ){
       log( "锟矫伙拷 %s 锟斤拷 %s 锟斤拷未锟斤拷锟斤拷锟斤拷\n", id , getGSName( ti ) );
       saacproto_ACKick_recv( ti, id, 6, -1);
    } else {
        log( "锟矫伙拷 %s 锟斤拷 %s 锟斤拷锟斤拷锟斤拷锟斤拷\n", id , getGSName( ti ) );
#ifdef _WAEI_KICK
...
#ifdef _ALLDOMAN					// (锟斤拷锟缴匡拷) Syu ADD 锟斤拷锟叫帮拷NPC
	...
	case 6:		//MEMORY UNLOCK; UNLOCK
		{
			...
			if( strcmp( result , SUCCESSFUL ) == 0 ){
				log( "锟斤拷锟斤拷锟剿猴拷 %s 锟缴癸拷锟斤拷\n" , id );
			} else {
				log( "锟斤拷锟斤拷锟剿猴拷 %s 失锟杰ｏ拷\n" , id );
				return;
			}
		...
		break;
		case 10:	//锟斤拷锟斤拷锟斤拷锟斤拷锟剿诧拷锟斤拷load锟斤拷锟斤拷锟斤拷
		{
			int i;
			for(i=0;i<MAXCONNECTION;i++)	{
...
int UNlockM_UnlockPlayer( void)
{
	...
	for( i=0; i<MAXUNlockM; i++)	{
		...
		if( isLocked( UNlockM[i].PlayerId ) ){
			log( "锟饺达拷锟斤拷锟斤拷锟斤拷锟�: %s 锟斤拷锟斤拷锟斤拷锟斤拷!!\n", UNlockM[i].PlayerId);
		}else{
			log( "锟饺达拷锟斤拷锟斤拷锟斤拷锟�: %s 锟窖撅拷锟斤拷锟斤拷!!\n", UNlockM[i].PlayerId);
		}
		reset_UNlockMPlayer( i);
		nums++;
	}
	//log( "锟饺达拷锟斤拷锟斤拷锟斤拷锟�: 锟杰硷拷 %d 锟斤拷锟矫伙拷锟斤拷锟斤拷 !!\n", nums);
	return nums;
}
...
void saacproto_ACCharLogin_recv( int fd, int clifd, char* id, char* pas, char* ip )
{
	...
	//   printf("锟斤拷陆锟斤拷息锟叫达拷锟斤拷\n");
	//   saacproto_ACCharLogin_send( fd, clifd, 0);
	//   return;
	// }
	//user_lock을 만들어도 오류나서 삭제
	// if (sasql_chehk_lock(id)){
	// 	printf("锟斤拷锟剿猴拷%s锟斤拷止锟斤拷陆!\n",id);
	//   saacproto_ACCharLogin_send( fd, clifd, 0);
	//   return;
	// }
	// if (sasql_chehk_lock(ip)){
	// 	printf("锟斤拷IP%s锟斤拷止锟斤拷陆!\n",ip);
	//   saacproto_ACCharLogin_send( fd, clifd, 0);
	//   return;
	// }
	printf("거쳤나??");
		...
		case 0:
			if(strlen(id)>0)
				if(sasql_del_lock(id))
					log("锟缴癸拷锟斤拷锟斤拷锟斤拷锟斤拷撕锟�:%s\n",id);
			if(strlen(ip)>0)
				if(sasql_del_lock(ip))
					log("锟缴癸拷锟斤拷锟斤拷锟斤拷锟絀P:%s\n",ip);
			break;
		case 1:
			if(strlen(id)>0)
				if(!sasql_chehk_lock(id))
					if(sasql_add_lock(id))
						log("锟缴癸拷锟斤拷锟斤拷锟剿猴拷:%s\n",id);
			if(strlen(ip)>0)
				if(!sasql_chehk_lock(ip))
